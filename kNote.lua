local L = AceLibrary("AceLocale-2.2"):new("kNote")
kNote = AceLibrary("AceAddon-2.0"):new("AceEvent-2.0", "AceHook-2.1", "AceDebug-2.0", "AceConsole-2.0", "AceDB-2.0")

local deformat = AceLibrary("Deformat-2.0")
local BC = AceLibrary("Babble-Class-2.2")

function kNote:OnInitialize()
	--self:SetDebugging(true)

	self:RegisterDB("kNoteDB")
	...
	self:RegisterChatCommand({"/kNote", "/auld"}, {
		type = "group",
		name = L["kNote"],
		desc = L["Options for kNote."],
		args = {
			data = {
	...
	for i=1, FRIENDS_TO_DISPLAY, 1 do
		table.insert(self.friendbuttons, self:CreateButton("kNote_FriendNote"..i, getglobal("FriendsFrameFriendButton"..i), "RIGHT", "RIGHT", -5, 0))
		self.friendbuttons[i].type = "friend"
	end
	for i=1, IGNORES_TO_DISPLAY, 1 do
		table.insert(self.ignorebuttons, self:CreateButton("kNote_IgnoreNote"..i, getglobal("FriendsFrameIgnoreButton"..i), "RIGHT", "RIGHT", -5, 0))
		self.ignorebuttons[i].type = "ignore"
	end
	for i=1, GUILDMEMBERS_TO_DISPLAY, 1 do
		table.insert(self.guildbuttons, self:CreateButton("kNote_GuildNote"..i, getglobal("GuildFrameButton"..i), "RIGHT", "RIGHT", 0, 0))
...
function kNote:CHAT_MSG_SYSTEM(message)
...
function kNote:ListenFor(which, stop)
	if stop then
...
function kNote:ScanTooltip(object, unitid)
	self:Debug(unitid)
...
function kNote:CanMessWithGameTooltip()
...
function kNote:AddToGameTooltipAndFixHeight(text, r, g, b)
...
function kNote:ClearData()
	self.db.realm.chars = {}
end

function kNote:CTImport()
...
function kNote:CreateButton(name, parent, point, relativePoint, x, y)
...
function kNote:ButtonClick(button)
...
function kNote:ButtonEnter(button)
...
function kNote:ButtonLeave(button)
	GameTooltip:Hide()
end

function kNote:CreateEditFrame()
	local f = CreateFrame("Frame", "kNote_Edit", UIParent)
	...
	local h_text = f:CreateFontString("kNote_EditHeaderText", nil, "GameFontNormal")
	h_text:SetPoint("TOP", h, "TOP", 0, -14)
	h_text:SetText(L["Edit note"])

	local edit_text = f:CreateFontString("kNote_EditDescText", nil, "GameFontNormal")
...
function kNote:SaveNote(name, note, ntype)
...
function kNote:SaveFriends(slotnum)
	local slot
...
function kNote:LoadFriends(slotnum)
	local slot
	if slotnum == "undo" then
		slot = self.undo
	else
...
function kNote:GetRankName(rank, sex)
